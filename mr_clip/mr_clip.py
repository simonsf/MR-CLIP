...
def matrix_diag(t):
    ...
    diag_mask = rearrange(
        i_range, 'i -> i 1') == rearrange(j_range, 'j -> 1 j')
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim=dim, dim_head=dim_head,
                        heads=heads, causal=causal, dropout=attn_dropout)),
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.abs_pos_emb = nn.Embedding(
            max_seq_len, dim) if not rotary_pos_emb else None
        self.rotary_pos_emb = RotaryEmbedding(
            min(dim_head, 32)) if rotary_pos_emb else None

        self.cls_token = nn.Parameter(torch.randn(dim)) if not causal else None

        self.transformer = Transformer(
            dim, dim_head=dim_head, causal=causal, **kwargs)
...
class ChannelSpatialAttention(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        # 通道注意力
        self.fc1 = nn.Conv3d(in_channels, in_channels //
                             reduction_ratio, 1, bias=False)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv3d(in_channels // reduction_ratio,
                             in_channels, 1, bias=False)

        # 空间注意力
        self.spatial_conv = nn.Conv3d(2, 1, kernel_size=(
            3, 3, 3), padding=(1, 1, 1), bias=False)

        self.norm = nn.LayerNorm([in_channels, 6, 8, 8], eps=1e-12)

        self.sigmoid = nn.Sigmoid()

    def forward(self, x, do_ca=False):
        ...
        spatial_max_out = torch.max(x, dim=1, keepdim=True)[0]
        spatial_concat = torch.cat([spatial_avg_out, spatial_max_out], dim=1)
        spatial_attention = self.sigmoid(self.spatial_conv(spatial_concat))

        # 结合通道与空间注意力
        out = spatial_attention * x
        out = out.permute(0, 2, 3, 4, 1).contiguous()
        # print('CAM: ',spatial_avg_out.shape, spatial_max_out.shape, out.shape)
...
class CPCAChannelAttention(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels):
        ...
        self.fc1 = nn.Conv3d(in_channels=input_channels,
                             out_channels=hidden_channels, kernel_size=1, stride=1, bias=True)
        self.fc2 = nn.Conv3d(in_channels=hidden_channels,
...
class MRCLIP(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # whether to pass in text mask to text encoder
        ...
        # self.CSAM = ChannelSpatialAttention(dim_latent)
        ...
        self.to_text_latent = nn.Linear(dim_text, dim_latent, bias=True)
        ...
        if downsample_image_embeds:
            ...
            self.to_visual_latent = nn.Sequential(
                RearrangeImage(),
                nn.Conv3d(dim_conv, dim_conv, 4, stride=2,
                          padding=1, bias=False, groups=dim_conv),
                nn.Conv3d(dim_conv, dim_latent, 1),
                Rearrange('b c h w z -> b (h w z c)'),
                nn.Linear(dim_image, dim_latent, bias=False)
            )
        else:
            self.to_visual_latent = nn.Linear(
                dim_image, dim_latent, bias=False)
            # self.to_visual_latent = nn.Linear(
            #     dim_latent, dim_latent, bias=False)
    ...
    def load(self, path, map_location=None):
        path = Path(path)
        assert path.exists()
        # 添加map_location参数到torch.load调用,指定gpu加载
    ...
    def forward(
        ...
        # image encoder is not trained if this is set to True, proposed by LiT paper
        ...
        if return_loss:
            ...
            text_ssl_loss = self.mlm(
                text.input_ids, attention_mask=text.attention_mask) if self.use_mlm else 0
            image_ssl_loss = self.visual_ssl(
                image) if self.use_visual_ssl else 0
        ...
        assert not (
            not return_loss and is_multiview), 'do not pass in augmented texts or images if not training'
        assert not (self.multiview_loss_weight ==
                    0 and is_multiview), 'multiview loss weight cannot be 0 if augmented text or images passed in'
        ...
        # print('0.1,', enc_text.is_contiguous())
        ...
        if self.text_causal_mask:
            ...
            assert torch.all(torch.any(eos_text_mask, dim=-1)
            ...
            eos_text_mask = torch.zeros_like(
                eos_text_mask).scatter(1, eos_indices, 1.).bool()
            ...
            enc_text = torch.cat((eos_tokens, rest_tokens), dim=1).contiguous()
            # print('0.11,', eos_tokens.is_contiguous())
        # print('0.2,', enc_text.is_contiguous())
        ...
        # enc_image_copy=rearrange(enc_image, 'b z y x d -> b (z y x) d') # B, Ps, D
        enc_image_copy=enc_image.clone()
        # enc_image = self.CSAM(enc_image)
        enc_image = enc_image.view(enc_image.shape[0], -1)
        # print(enc_image.shape)
        # print('1,', enc_image.is_contiguous())
        ...
        if self.use_all_token_embeds:
            ...
            text_embeds = enc_text[:,
                                   1:] if self.text_has_cls_token else enc_text
            image_embeds = enc_image[:,
                                     1:] if self.visual_has_cls_token else enc_image
        else:
            text_embeds = enc_text[:, :] if enc_text.ndim == 3 else enc_text
            image_embeds = enc_image[:,
                                     :] if enc_image.ndim == 3 else enc_image

        # project to latents
        #text_embeds = text_embeds.view(text_embeds.shape[0], -1)
        text_embeds = text_embeds[:, 0, :].contiguous()

        # print('2,', text_embeds.is_contiguous())
        # print('3,', image_embeds.is_contiguous())
        ...
        # print('4,', text_latents.is_contiguous())
        # print('5,', image_latents.is_contiguous())

        text_latents, image_latents = map(
            l2norm, (text_latents, image_latents))
        ...
        if return_latents:
            ...
            return text_latents, image_latents, enc_image_copy

        text_latents_copy = text_latents.clone()
        image_latents_copy = image_latents.clone()
        # image_latents=image_latents.mean(-1)

        # early return, if needed

        if not return_loss and self.use_all_token_embeds:
            einsum_args = (text_latents_extra, image_latents_extra) if self.extra_latent_projection and not text_to_image else (
        ...
        if not return_loss and not self.use_all_token_embeds:
            # print('Pred Output Size:', text_latents.size())
            einsum_args = (text_latents_extra, image_latents_extra) if self.extra_latent_projection and not text_to_image else (
                text_latents, image_latents)
        ...
        if self.use_all_token_embeds:
            ...
            sim_text_to_image = einsum(
                'm x t d, n y i d -> m n x y t i', text_latents, image_latents) * temp

            sim_image_to_text = sim_text_to_image
            if self.extra_latent_projection:
                sim_image_to_text = einsum(
                    'm x t d, n y i d -> m n x y t i', text_latents_extra, image_latents_extra) * temp

            text_to_image = reduce(
                sim_text_to_image, '... t i -> ... t', 'max')
            text_to_image_mask = rearrange(
                text_mask, '(m b) t -> m 1 b 1 t', m=num_batch_texts).bool()
            text_to_image = masked_mean(
                text_to_image, text_to_image_mask, dim=-1)

            image_to_text_mask = rearrange(
                text_mask, '(m b) t -> m 1 b 1 t 1', m=num_batch_texts).bool()
            ...
            text_to_image = einsum(
                'm t d, n i d -> m n t i', text_latents, image_latents) * temp
            image_to_text = rearrange(text_to_image, '... t i -> ... i t')

            if self.extra_latent_projection:
                image_to_text = einsum(
                    'm t d, n i d -> m n i t', text_latents_extra, image_latents_extra) * temp
        ...
        if self.decoupled_contrastive_learning:
            ...
            text_to_image_exp, image_to_text_exp = map(lambda t: t.masked_fill(
                pos_mask, 0.), (text_to_image_exp, image_to_text_exp))
        ...
        text_to_image_loss = (-log(text_to_image_pos) +
                              log(text_to_image_denom)).mean(dim=-1)
        image_to_text_loss = (-log(image_to_text_pos) +
                              log(image_to_text_denom)).mean(dim=-1)
        ...
        cl_loss_weight = 1 - (self.text_ssl_loss_weight +
                              self.image_ssl_loss_weight + multiview_loss_weight)
        ...
        return loss, image_latents_copy, text_latents_copy
